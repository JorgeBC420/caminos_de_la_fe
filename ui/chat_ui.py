from ursina import *

class ChatUI(Entity):
    def __init__(self, manager, channel='global', **kwargs):
        super().__init__(parent=camera.ui, **kwargs)
        self.manager = manager
        self.channel = channel
        self.is_maximized = False
        
        # Vista minimizada (siempre visible)
        self.minimized_view = Entity(parent=self, y=-0.35, x=-0.3)
        self.chat_lines = [Text(parent=self.minimized_view, text="", y=-i*0.04) for i in range(3)]

        # Vista maximizada (se activa al hacer clic)
        self.maximized_view = Entity(parent=self, enabled=False, model='quad', scale=(0.8, 0.5), color=color.black90)
        self.chat_history = Text(parent=self.maximized_view, text="Historial de Chat...")
        self.chat_input = InputField(parent=self.maximized_view, y=-0.4)

        self.panel = Entity(parent=self, model='quad', scale=(0.7, 0.5), color=color.gray)
        self.input = InputField(parent=self.panel, y=-0.2)
        Button(parent=self.panel, text="Enviar", y=-0.3, on_click=self.send_message)
        self.messages_panel = Entity(parent=self.panel)
        self.refresh_messages()

    def send_message(self):
        msg = self.input.text
        self.manager.send_message(self.channel, msg)
        self.input.text = ''
        self.refresh_messages()

    def refresh_messages(self):
        destroy(self.messages_panel)
        self.messages_panel = Entity(parent=self.panel)
        messages = self.manager.fetch_messages(self.channel)
        for i, m in enumerate(messages[-10:]):
            Text(parent=self.messages_panel, text=f"{m['player']}: {m['message']}", y=0.2-i*0.05, x=-0.3)

    def add_message(self, message):
        # Lógica para añadir un mensaje a la lista y actualizar las vistas
        pass

    def toggle_view(self):
        self.is_maximized = not self.is_maximized
        self.maximized_view.enabled = self.is_maximized
